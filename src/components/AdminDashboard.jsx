...
export default function AdminDashboard({ session, go }) {
  ...
  const saveProduct = async (event) => { event.preventDefault(); let imageUrl = form.image_url; if (imageFile) { const safe = imageFile.name.toLowerCase().replace(/[^a-z0-9._-]/g, "-"); const path = Date.now() + "-" + safe; const { error: uploadError } = await supabase.storage.from("product-images").upload(path, imageFile, { upsert: false, contentType: imageFile.type }); if (uploadError) { setMessage(uploadError.message); return; } imageUrl = supabase.storage.from("product-images").getPublicUrl(path).data.publicUrl; } if (!imageUrl) { setMessage("Upload an image or paste an image URL."); return; } const gallery = form.gallery_text.split(/\r?\n/).map((value) => value.trim()).filter(Boolean); const colors = form.colors_text.split(/\r?\n/).map((row) => { const [name, hex, image] = row.split("|").map((value) => value?.trim()); return name ? { name, hex: hex || "#dddddd", image: image || imageUrl } : null; }).filter(Boolean); const payload = { ...form, image_url: imageUrl, gallery, colors, price: Number(form.price), stock: Number(form.stock), discount: Number(form.discount) }; delete payload.id; delete payload.gallery_text; delete payload.colors_text; const { error } = form.id ? await supabase.from("products").update(payload).eq("id", form.id) : await supabase.from("products").insert(payload); setMessage(error?.message || (form.id ? "Product updated." : "Product added to the storefront.")); if (!error) { setForm(emptyProduct); setImageFile(null); setFormOpen(false); load(); } };
          ...
          <article key={order.id} className="admin-order-card">
            <header className="admin-order-card-header">
              <div>
                ...
                <span className="admin-order-date">
                  {order.created_at ? new Date(order.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "Recent"}
                </span>
              </div>
              <div className="admin-order-header-right">
                <span className="admin-order-pay-badge">
                  💳 {order.payment_method?.toUpperCase() || "ADVANCE"}
                </span>
                <strong className="admin-order-total">PKR {Number(order.total).toLocaleString()}</strong>
              </div>
            </header>
            ...
            <div className="admin-order-customer-box">
              <div className="admin-order-customer-info">
                ...
                {waLink && (
                  <a href={waLink} target="_blank" rel="noreferrer" className="admin-order-wa-link">
                    💬 WhatsApp
                  </a>
                )}
              </div>
              <div className="admin-order-address-box">
                ...
                <p className="address-text">{order.address || "No delivery address provided"}</p>
              </div>
            </div>

            <div className="admin-order-items-box">
              <span className="items-label">📦 Ordered Items:</span>
              <div className="admin-order-items-list">
                {order.order_items?.map((item, idx) => (
                  <div key={item.id || idx} className="admin-order-item-row">
                    <span>{item.product_name}</span>
                    <strong>× {item.quantity} (PKR {Number((item.unit_price || 0) * (item.quantity || 1)).toLocaleString()})</strong>
                  </div>
                ))}
              </div>
            </div>
