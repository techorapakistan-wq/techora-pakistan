...
import techoraLogo from "./assets/techora-logo.png";
import "./App.css";
...
export default function App() {
  ...
  useEffect(() => { if (!supabase) return; supabase.auth.getSession().then(({ data }) => setSession(data.session)); const { data: listener } = supabase.auth.onAuthStateChange((_event, nextSession) => setSession(nextSession)); return () => listener.subscription.unsubscribe(); }, []);
  useEffect(() => { if (!supabase) return; const loadProducts = () => supabase.from("products").select("*").eq("is_active", true).order("created_at", { ascending: false }).then(({ data }) => { if (data?.length) { const dbProducts = data.map(item => ({ ...item, image: item.image_url, gallery: Array.isArray(item.gallery) ? item.gallery : [], colors: Array.isArray(item.colors) ? item.colors : [], price: Number(item.price) * (1 - Number(item.discount || 0) / 100) })); const existingIds = new Set(dbProducts.map(p => p.id)); const extraFallback = fallbackProducts.filter(p => !existingIds.has(p.id)); setProducts([...dbProducts, ...extraFallback]); } }); loadProducts(); const timer = window.setInterval(loadProducts, 15000); return () => window.clearInterval(timer); }, []);
  ...
  const nav = [["Home", "/"], ["Shop", "/shop"], ["Categories", "/shop"], ["New arrivals", "/shop"], ["Best sellers", "/shop"]];
...
function Home({go,openProduct,products,add,wish,toggleWish,session}) {
  const categories=products.map(product => [product.category, product.name, product.image]);
  const best=products.slice(0,4), fresh=products.length > 4 ? products.slice(4,8) : products.slice().reverse();
...
function Product({product,add,wished,toggleWish,go,session,onRatingUpdate}) {
  ...
  const configuredProduct = { ...product, selectedColor };
  const chooseColor = (color) => {
    setSelectedColor(color.name);
    if (color.image) setCurrentImage(color.image);
  };
  return <main className="page-shell product-page">
    <div className="breadcrumb"><button onClick={()=>go("/shop")}>Shop</button> <span>/</span> {product.name}</div>
    <div className="product-detail-page">
      <div className="product-gallery">
        <div className="product-detail-image"><AnimatePresence mode="wait"><motion.img key={currentImage} src={currentImage} alt={product.name + (selectedColor ? " in " + selectedColor : "")} initial={{opacity:.25}} animate={{opacity:1}} exit={{opacity:.2}} transition={{duration:.2}}/></AnimatePresence></div>
        {gallery.length > 1 && <div className="product-thumbnails" aria-label="Product images">{gallery.map((image,index)=><button className={currentImage===image?"active":""} key={image} onClick={()=>setCurrentImage(image)} aria-label={"View image " + (index+1)}><img src={image} alt=""/></button>)}</div>}
      </div>
      <div className="product-detail-copy">
        <p className="eyebrow">{product.category}</p><h1>{product.name}</h1>
        <div className="detail-price">{Number(product.discount || 0) > 0 && <del>{money(originalPrice(product))}</del>}<strong>{money(product.price)}</strong>{Number(product.discount || 0) > 0 && <span>{product.discount}% off</span>}</div>
        <p>{product.description}</p>
        {colors.length > 0 && <fieldset className="colour-picker"><legend>Choose colour <strong>{selectedColor}</strong></legend><div>{colors.map(color=><button type="button" key={color.name} className={selectedColor===color.name?"active":""} onClick={()=>chooseColor(color)} aria-label={"Choose " + color.name} aria-pressed={selectedColor===color.name}><span style={{backgroundColor:color.hex || "#ddd"}}/><small>{color.name}</small></button>)}</div></fieldset>}
        <p className={product.stock===0?"stock-state sold-out":"stock-state"}>{product.stock===0?"Unavailable":"In stock — ready to order"}</p>
...
function OrderForm({items,session,close,onComplete}) {
...
function Orders({go}) { return <main className="page-shell simple-page"><p className="eyebrow">MY ACCOUNT</p><h1>Your <em>orders.</em></h1><div className="orders-empty"><Icon name="bag"/><h3>No orders yet</h3><p>When you place an order, its details will appear here.</p><button className="button button-ink" onClick={()=>go("/shop")}>Start shopping <Icon name="arrow"/></button></div></main> }
function Story({go}) { return <main className="page-shell story-page"><p className="eyebrow">OUR STORY</p><h1>Technology for a more <em>considered</em> life.</h1><p>Techora brings together useful, dependable technology with the personal service of a local Pakistani store.</p><button className="button button-ink" onClick={()=>go("/shop")}>Explore the edit <Icon name="arrow"/></button></main> }
function Footer({go}) { return <footer><div className="footer-main"><div><button className="wordmark" onClick={()=>go("/")}><img src={techoraLogo} alt="Techora Gadgets"/></button><p>Considered technology for modern life.</p></div><div><p className="footer-title">Explore</p><button onClick={()=>go("/shop")}>Shop</button><button onClick={()=>go("/story")}>Our story</button><button onClick={()=>go("/reviews")}>Reviews</button><button onClick={()=>go("/orders")}>My orders</button></div><div><p className="footer-title">Customer service</p><a href="https://wa.me/923229701332?text=Hi%20Techora%2C%20I%20need%20delivery%20information." target="_blank" rel="noreferrer">Delivery information</a><a href="mailto:techorapakistan@gmail.com">Email support</a></div><div><p className="footer-title">Follow along</p><div className="footer-socials"><a className="social-instagram" href="https://www.instagram.com/techorapakistan?stkn=MXNsb2diNmt5Yzh6ZA==" target="_blank" rel="noreferrer" aria-label="Instagram"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" fill="none" stroke="currentColor" strokeWidth="1.7"/><circle cx="12" cy="12" r="4" fill="none" stroke="currentColor" strokeWidth="1.7"/><circle cx="17.4" cy="6.6" r="1" fill="currentColor"/></svg></a><a className="social-facebook" href="https://www.facebook.com/share/1fLZux37nz/" target="_blank" rel="noreferrer" aria-label="Facebook">f</a><a className="social-tiktok" href="https://www.tiktok.com/@techorapakistan" target="_blank" rel="noreferrer" aria-label="TikTok"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14.5 3c.4 2.4 1.8 3.9 4.1 4.2v3.1c-1.5 0-2.8-.4-4.1-1.2v5.9a5.4 5.4 0 1 1-4.7-5.4v3.2a2.3 2.3 0 1 0 1.6 2.2V3h3.1Z" fill="currentColor"/></svg></a><a className="social-whatsapp" href="https://wa.me/923229701332" target="_blank" rel="noreferrer" aria-label="WhatsApp"><Icon name="whatsapp"/></a></div></div></div><div className="footer-bottom"><span>© 2026 Techora Pakistan</span><div className="footer-payments"><img src={easypaisaLogo} alt="EasyPaisa"/><img src={jazzcashLogo} alt="JazzCash"/><span className="visa">VISA</span></div></div></footer> }
